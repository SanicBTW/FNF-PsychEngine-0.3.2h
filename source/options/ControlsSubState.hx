...
class ControlsSubState extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float)
	{
		if (!rebindingKey)
		{
			...
			if (controls.ACCEPT && nextAccept <= 0)
			{
				...
				{
					SaveData.keyBinds = SaveData.defaultKeys.copy();
					reloadKeys();
					changeSelection();
		...
		else
		{
			...
			if (keyPressed > -1)
			{
				var keysArray:Array<FlxKey> = SaveData.keyBinds.get(optionShit[curSelected][1]);
				...
				SaveData.keyBinds.set(optionShit[curSelected][1], keysArray);
	...
	{
		var keys:Array<Dynamic> = SaveData.keyBinds.get(optionShit[num][1]);
	...
	function reloadKeys()
	{
		...
		trace('Reloaded keys: ' + SaveData.keyBinds);
